...
import com.vcans.dao.SCTouPeiDao;
import com.vcans.model.BandleParam;
import com.vcans.model.MyProtocol;
import com.vcans.model.ToupeiInfo;
import com.vcans.tool.ActivityHelper;

public class DPTPUploadProcessor extends UploadProcessor {

	public SCTouPeiDao mSCToupeiDao=null;
	...
	public DPTPUploadProcessor()
	{
		mSCToupeiDao=new SCTouPeiDao();
	}
	...
	public void displayDetail(List<HashMap<String,Object> > mapInfo)
	{
		...
		int[] szRId={R.id.tvRZPlanIdv,R.id.tvRZFactoryIdv,R.id.tvProTypeName,R.id.tvProTypeId,R.id.tvGongXuId,R.id.tvGongXuName,
		...
		mBdParam.rid=R.layout.dp_sctoupei_item_detail;
	...
	public void SaveToupeiInfo()
	{
		...
		CheckBox chkBackModify=(CheckBox)mActParent.findViewById(R.id.chkBackModify);
		CheckBox chkInitPeiBuTR=(CheckBox)mActParent.findViewById(R.id.chkInitPBTR);
		...
		toupei.strBackModify=chkBackModify.isChecked()?"Y":"N";
		toupei.strInitPeiBuTouRu=chkInitPeiBuTR.isChecked()?"Y":"N";
		...
		if(toupei.strPiCi.equals("")||toupei.nPiShu.equals("")||toupei.nNum.equals(""))
		{
	...
	public Vector<Vector<String>> GetSendData(String[] extrData) {
		...
		for(HashMap<String,Object> mapToupei:mTouPeiList)
		{
			...
			//采集器内部内部ID,染整计划号,染整厂编码,工序编号,工序名称,公司订单号,采购合同号,物料编号,物料描述,
			//存货描述,存货标志,原库存匹数,原库存数量,回修,初始坯布投入,匹数,数量,用户名,是否复合布(Y/N]
			...
			child.add((String)mapToupei.get("BackModify"));
			child.add((String)mapToupei.get("InitPeiBuTR"));
			child.add((String)mapToupei.get("PiShu"));
			child.add((String)mapToupei.get("Num"));
			child.add((String)mapToupei.get("UserId"));
			child.add("N");
			child.add((String)mapToupei.get("ProColor"));
	...
	public MyProtocol GetProtocol() {
		...
		p.sendCmd0= DPProtocol.m_vfxVAG4BDePei_ShengChanHuiBaoTouLiaoBiao0;
		p.sendCmd1=DPProtocol.m_vfxVAG4BDePei_ShengChanHuiBaoTouLiaoBiao1;
		p.sendCmd2=DPProtocol.m_vfxVAG4BDePei_ShengChanHuiBaoTouLiaoBiao2;
		p.receCmd0=DPProtocol.m_vfxVAG4BDePei_ShengChanHuiBaoTouLiaoBiaoRe0;
		p.receCmd1=DPProtocol.m_vfxVAG4BDePei_ShengChanHuiBaoTouLiaoBiaoRe1;
		p.receCmd3=DPProtocol.m_vfxVAG4BDePei_ShengChanHuiBaoTouLiaoBiaoRe3;
		p.receCmd5=DPProtocol.m_vfxVAG4BDePei_ShengChanHuiBaoTouLiaoBiaoRe5;
