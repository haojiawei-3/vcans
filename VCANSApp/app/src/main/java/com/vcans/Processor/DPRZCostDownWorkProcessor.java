...
public class DPRZCostDownWorkProcessor extends DownloadProcessor {
	...
	public void InitDropWorkProcess()
	{
		...
		ActivityHelper.lstGongyi.clear();//费用选择
		...
		if(vctReceData.size()>0)
		{
			for(Vector<String> record:vctReceData)
			{
				if(record.size()>0)
				{
					...
					else if(record.get(0).equals("染整费用选择"))
					{
						if(record.size()<3)
							continue;
		...
		//ActivityHelper.InitDrop(mActParent, R.id.dropCostSelect, ActivityHelper.lstGongyi,true);
