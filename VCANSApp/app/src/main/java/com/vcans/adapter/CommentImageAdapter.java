...
public class CommentImageAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		// TODO Auto-generated method stub
		if (convertView == null) {
			convertView = inflater.inflate(R.layout.itemimg_comment, null);
			viewHolder = new ViewHolder();
			viewHolder.iv_user_photo = (ImageView) convertView
					.findViewById(R.id.iv_YPPic);
		...
		if(position == selectIndex){  
			viewHolder.LLayoutItem.setBackgroundDrawable(
					this.context.getResources().getDrawable(R.color.chengse));
        }else{  
		...
		String[] szFiles=ActivityHelper.mySplit(list_comment.get(position).get("YPValue").toString(), ";");
		...
		if(!strTMFile.equals(""))
			viewHolder.iv_user_photo.setImageBitmap(ActivityHelper.fileToBitmap(strTMFile));
		else
			viewHolder.iv_user_photo.setImageResource(R.drawable.no_img);
